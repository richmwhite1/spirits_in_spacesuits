#!/usr/bin/env python3
"""
Ingest 2010 sermon transcripts that contain an embedded YouTube URL on line 1.

File format (same as the 2009 batch):
...
Notes for the 2010 batch:
  * Several dates have a fully-titled transcript plus one or more short clip
    files named "<date> (1)", "<date> (2)", etc. Each clip is a DISTINCT
    YouTube video, so we keep the number in the title to avoid collisions
    (e.g. "2010-10-03 (1)") rather than stripping it.
  * One file ("2010-10-31 (1)") contains no transcript — only a leftover AI
    error message — and is skipped automatically.

Usage:
    python3 scripts/ingest_2010_transcripts.py
    python3 scripts/ingest_2010_transcripts.py --dry-run
"""
...
BASE_DIR    = Path('/Users/richardwhite/Downloads/drive-download-20260623T144249Z-3-001')
...
# Phrases that mark an AI artifact / non-transcript file to skip outright.
JUNK_MARKERS = (
    "I don't see any actual transcript content",
    "Please provide the actual homily transcript",
)
...
def parse_filename(stem: str) -> tuple[str, str | None]:
    ...
    # Capture a trailing "(1)", "(2)" clip number before we touch anything else.
    num_match = NUM_SUFFIX_RE.search(stem)
    clip_num = num_match.group(1) if num_match else None
    body = NUM_SUFFIX_RE.sub('', stem).strip() if num_match else stem
    ...
    if m:
        ...
        body = body[m.end():].strip().lstrip('- _').strip()

    title = re.sub(r'_+', ' ', body).strip()
    title = re.sub(r'\s{2,}', ' ', title).lstrip('- ').strip()

    if not title:
        # Generic clip file with no real title — keep date + clip number so
        # same-date clips stay distinct (e.g. "2010-10-03 (1)").
        base = date_str or stem
        title = f"{base} ({clip_num})" if clip_num else base
    elif clip_num:
        # Titled file that also carries a clip number — preserve both.
        title = f"{title} ({clip_num})"
...
def main():
    ...
    for fp in txt_files:
        ...
        if any(marker in raw_text for marker in JUNK_MARKERS):
            print(f"  JUNK  {fp.name[:70]}  (no transcript content — skipped)")
            total_skipped += 1
            continue
