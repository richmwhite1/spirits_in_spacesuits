...
const DAILY_LIMIT = parseInt(process.env.RATE_LIMIT_DAILY || '50');
...
export async function checkRateLimit(ip) {
  ...
  if (error) {
    // If RPC doesn't exist yet or DB error, fail open (allow request)
    // Better to let a few extra requests through than block legitimate users
...
export function rateLimitResponse() {
  return new Response(
    JSON.stringify({
      ...
      message: "You've reached the daily limit of questions. Come back tomorrow — the archive will still be here.",
